...
/* ----------> Functions for the general variables case. <---------- */
...
void calculate_Function_Values(double* input_values, double mu_value, numerical_Parameters &parameters, double* output) {
	// The alpha4 function value and its 3 derivative values (at the current coordinates/variables) are stroed for future reuse.
	// Calculation of the values of the reusable sum.
	...
	for (int k = 0; k < 3; k++) {
		...
		alpha4_derivative /= -parameters.alpha4;
		...
		output[k] = summand1 + sum * alpha4_derivative + 2 * mu_value * (summand2  + alpha4_derivative);
	}
	// Calculation of the 4th function value: differentiation with respect to Lagrange's coefficient.
	output[3] = 1;
	for (int i = 0; i < 4; i++) {
		for (int j = i + 1; j < 4; j++) {
			output[0] -= 2 * input_values[i] * input_values[j] * parameters.matrices[0][i][j];
		}
		output[3] -= input_values[i] * input_values[i];
	}
}
...
void calculate_Jacobi_Matrix(double* input_values, double mu_value, numerical_Parameters parameters, double (*output)[4]) {
	for (int k = 0; k < 3; k++) {
		...
		for (int m = k + 1; m < 3; m++) {
			output[k][m] = (parameters.matrices[0][m][k] + parameters.alpha4_derivatives[m] * parameters.matrices[0][3][k] +
				(-1 / parameters.alpha4) * (parameters.multi_use_sum + 1) * (parameters.alpha4_derivatives[k] * parameters.alpha4_derivatives[m] + parameters.matrices[0][m][k]) +
				parameters.alpha4_derivatives[k] * (parameters.matrices[0][m][3] + parameters.alpha4_derivatives[m] * parameters.matrices[0][k][3])) * 2 * mu_value;
			for (int n = 1; n < 4; n++) {
		...
		output[k][k] = (parameters.matrices[0][k][k] + parameters.alpha4_derivatives[k] * parameters.matrices[0][3][k] +
				(-1 / parameters.alpha4) * (parameters.multi_use_sum + 1) * (parameters.alpha4_derivatives[k] * parameters.alpha4_derivatives[k] + parameters.matrices[0][k][k]) +
				parameters.alpha4_derivatives[k] * (parameters.matrices[0][k][3] + parameters.alpha4_derivatives[k] * parameters.matrices[0][k][3])) * 2 * mu_value;
		for (int n = 1; n < 4; n++) {
...
/* ----------> Functions for the case, when the variables describe a quaternion "close" to the approximative hyperplane. <---------- */

// Function to calculate the values of the derivatives of the Lagrange function with given variables/coordinates as input (passed in as the input_values array).
// The calculated values are saved to the output array. The numerical parameters are passed in the numerical_Parameters struct. Some of the reusable parameters are also written into this struct.
void calculate_Alt_Function_Values(double* input_values, numerical_Parameters &parameters, double* output) {
	// Calculation of the values of the first 3 function values: differentiation with respect to the variables/coordinates.
	for (int k = 0; k < 3; k++) {
		double summand1 = 0, summand2 = 0;
		for (int i = 0; i < 3; i++) {
			...
			summand1 *= input_values[i + 1];
			summand2 += input_values[i + 1] * parameters.matrices[0][k][i];
		}
		output[k + 1] = summand1 + 2 * input_values[0] * summand2;
	}
	// Calculation of the 4th function value: differentiation with respect to Lagrange's coefficient.
	output[0] = 1;
	for (int i = 0; i < 3; i++) {
		for (int j = i + 1; j < 3; j++) {
			output[0] -= 2 * input_values[i + 1] * input_values[j + 1] * parameters.matrices[0][i][j];
		}
		output[0] -= input_values[i + 1] * input_values[i + 1] * parameters.matrices[0][i][i];
	}
}

// Function to calculate the Jacobi matrix of the transformation (R^4 -> R^4) defined by the 4 derivative functions at the current coordinates/variables (passed in as the input_values array).
// The calculate values are saved to the output 2D array. The numerical parameters and the reusable coefficients are passed in the numerical_Parameters struct.
void calculate_Alt_Jacobi_Matrix(double* input_values, numerical_Parameters parameters, double (*output)[4]) {
	for (int k = 0; k < 3; k++) {
		// The Jacobi matrix is symmetic, which is used to cut execution time.
		// Calculation of the left upper corner 3x3 block.
		for (int m = k + 1; m < 3; m++) {
			output[k + 1][m + 1] = parameters.matrices[0][m][k] * 2 * input_values[0];
			for (int n = 1; n < 4; n++) {
				output[k + 1][m + 1] += parameters.matrices[n][k][m] + parameters.matrices[n][m][k];
			}
			output[m + 1][k + 1] = output[k + 1][m + 1];
		}
		output[k + 1][k + 1] = parameters.matrices[0][k][k] * 2 * input_values[0];
		for (int n = 1; n < 4; n++) {
			output[k + 1][k + 1] += 2 * parameters.matrices[n][k][k];
		}
		// Calculation of the right upper 3x1 block.
		output[0][k] = 0;
		for (int i = 0; i < 3; i++) {
			output[0][k] += input_values[i + 1] * parameters.matrices[0][i][k];
		}
		output[0][k] *= 2;
		output[k][0] = output[0][k];
	}
	output[0][0] = 0;
}
...
// The numerical_Parameters struct contains the numerical data used to calculate the values of the derivative functions and the Jacobi matrix. The final variables/coordinates are stored in the output_coordinates array.
optimisation_Output Newtons_Algorithm(numerical_Parameters& parameters, TRIAD_Output quaternions) {
	...
	while ( result.iteration_count < 1 ||  result.delta > 1.0e-6 )
	{
		...
		for (int i = 0; i < 3; i++) {
			for (int j = i + 1; j < 3; j++) {
				parameters.alpha4 -= 2 * variables[i] * variables[j] * parameters.matrices[0][i][j];
			}
			parameters.alpha4 -= variables[i] * variables[i] * parameters.matrices[0][i][i];
		}
		parameters.alpha4 = sqrt(abs(parameters.alpha4));
		result.coordinates[3] = parameters.alpha4;
		// Selection of the variables case.
		/*if (parameters.alpha4 > 1.0e-7) {
			calculate_Function_Values(variables, parameters, function_values);
			calculate_Jacobi_Matrix(variables, parameters, Jacobi_Matrix);
		}
		else {
			calculate_Alt_Function_Values(variables, parameters, function_values);
			calculate_Alt_Jacobi_Matrix(variables, parameters, Jacobi_Matrix);
		}*/
		calculate_Function_Values(result.coordinates, variables[3], parameters, function_values);
		calculate_Jacobi_Matrix(result.coordinates, variables[3], parameters, Jacobi_Matrix);
		// Variables correction.
		calculate_Matrix_Inverse(Jacobi_Matrix, Jacobi_Inverse);
		for (int i = 0; i < 3; i++) { variables[i] = result.coordinates[i]; }
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				variables[i] -= Jacobi_Inverse[i][j] * function_values[j];
			}
		}
		for (int i = 0; i < 3; i++) { result.coordinates[i] = variables[i + 1]; }
		result.coordinates[3] = 1;
		for (int i = 0; i < 3; i++) {
			for (int j = i + 1; j < 3; j++) {
				result.coordinates[3] -= 2 * variables[i] * variables[j] * parameters.matrices[0][i][j];
			}
			result.coordinates[3] -= variables[i] * variables[i] * parameters.matrices[0][i][i];
		}
